...
import com.demo.poc.entrypoint.contactdata.mapper.ContactDataRequestMapper;
...
public class ContactDataServiceImpl implements ContactDataService, StateService {
  ...
  private final ContactDataRequestMapper requestMapper;
  private final ContactDataResponseMapper responseMapper;
  private final StateBuilder stateBuilder;

  @Override
  public Uni<ContactDataResponseDto> getContactData(ContactDataRequestDto request, String channelId) {
    ...
    return Uni.combine().all()
        ...
        .map(context -> (ProductDeliveryContext) this.processState(context))
        .flatMap(context -> Uni.createFrom().item(stateBuilder.of(context).buildAsResponse())
            .map(contextResponse -> responseMapper.toResponse(contextResponse, context.getContactData().getAddresses())));
  }
  ...
  @Override
  public Context processState(Context context) {
    return stateBuilder
        .init(context)
